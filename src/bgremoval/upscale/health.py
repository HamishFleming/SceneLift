...
from .registry import create_upscaler
...
def _close_upscaler(upscaler: Any) -> None:
...
def run_healthcheck(input_source: str, method: str) -> tuple[HealthCheckResult, HealthCheckResult]:
    ...
    start = time.perf_counter()
    try:
        output = upscaler.upscale(frame.copy())
        ...
        return (
            HealthCheckResult("backend", "ok", load_elapsed, details={"method": method, "loader": type(upscaler).__name__}),
            HealthCheckResult(
                "inference",
                "ok",
                infer_elapsed,
                details={"method": method, "output_shape": getattr(output, "shape", None)},
            ),
        )
    except Exception as exc:
        infer_elapsed = (time.perf_counter() - start) * 1000.0
...
def benchmark_upscaler(
    ...
    *,
    ...
    return benchmark_upscaler_instance(
        upscaler,
        frame,
        method=method,
        load_ms=load_ms,
...
def benchmark_upscaler_instance(
    upscaler: Any,
    frame: np.ndarray,
    *,
    method: str,
